...
public class PlayerControl : MonoBehaviour
{
    ...
    void InitialisePlayerStartPosition()
    {
        ...
        float XPlayerPitchRange = TheOppoGoalArea.transform.position.x - TheOwnGoalArea.transform.position.x;    // Expect this Range to be 26 [-13 to +13]  with 13 at centre
        ...
        {
            // Red Team Heavy Penalise at the Low Game Levels 
            RunSpeed = 2.5f;  // Deafult Run Speed
            if (TheCurrentGameLevel == 1)
            {
                RunSpeed = 0.1f;
                if (OwnIdentity == 21)
    ...
    void Update()
    {
        ...
        {
            // S - Pass the Ball
            if (Input.GetKey(KeyCode.S) && CheckIfAbleToStrike())
            {
                // Check and Perfom Srike The Ball Toward Goal
                KickTarget = PickAGoalAimPoint();
                transform.LookAt(new Vector3(KickTarget.x,transform.position.y,KickTarget.z));
                if (ThePlayerCurrentState != PlayerState.StrikingTheBall) SetStrikingTheBall();
            }   // S -Strike Ball
            
            // P - Pass the Ball
            if (Input.GetKey(KeyCode.P) && CurrentlyHasTheBall)
            {
                KickTarget = TeamPlayer.transform.position;
                transform.LookAt(new Vector3(KickTarget.x, transform.position.y, KickTarget.z));
                if(ThePlayerCurrentState != PlayerState.PassingBall)  SetPassingTheBall();
            }   // P - Pass the Ball

            // F- Dribble Forward
            if (Input.GetKey(KeyCode.F) && CurrentlyHasTheBall)
            {
                if (ThePlayerCurrentState != PlayerState.DribblingForward) SetDribblingForward();
            }   // F- Dribble Forward

            // A- Goto To Attack Position
            if (Input.GetKey(KeyCode.A) && !CurrentlyHasTheBall)
            {
                if (ThePlayerCurrentState != PlayerState.RunningToAttack) SetGoToAttack();
            }   // A-Goto To Attack Position

            // D- Goto To Defence Position
            if (Input.GetKey(KeyCode.D) && !CurrentlyHasTheBall)
            {
                if (ThePlayerCurrentState != PlayerState.RunningToDefence) SetGoToDefence(); 
            }   // D-Goto To Defence Position

            // B- Goto To Ball Player
            if (Input.GetKey(KeyCode.B) && !CurrentlyHasTheBall)
            {
                if (ThePlayerCurrentState != PlayerState.RunningToBall) SetRunningToBall(); 
            }
            // B - Goto To Ball Player
    ...
    // =========================================================================================
    // Main Player Processing Loop 
    void FixedUpdate()
    {
        // Main Player Cycle Loop 
        ...
        if ((!UnderManualControl) && !(CelebrationPeriod))
        {
            ...
            if (DecisionCountDown == 0)
            {
                ...
                {
                    // Request a New Tactical Action Review
                    ProposedAction = ReviewTacticalActions();
            ...
            if ((ProposedAction == TacticalActions.ChaseBallPlayer) && (ThePlayerCurrentState != PlayerState.RunningToBall)) SetRunningToBall();
            if ((ProposedAction == TacticalActions.DribbleWide) && (ThePlayerCurrentState != PlayerState.DribblingForward)) SetDribblingWide();
            if ((ProposedAction == TacticalActions.DribbleForward) && (ThePlayerCurrentState != PlayerState.DribblingForward)) SetDribblingForward();
            ...
            if ((ProposedAction == TacticalActions.Goto_Attack) && (ThePlayerCurrentState != PlayerState.RunningToAttack) )
            {
            ...
            if ((ProposedAction == TacticalActions.Goto_Defence) && (ThePlayerCurrentState != PlayerState.RunningToDefence))
            {
        ...
        {
            //==============================================
            // Normal Automatic Player Movement 
            ...
            {     
                // Need Rotate Dribble Towrads Opposite Goal Area
        ...
        // =================================================
        // Check Animations Progress Events
        CurrentAnimationName = ThePlayerAnimator.GetCurrentAnimatorClipInfo(0)[0].clip.name;
   ...
   // ===========================================================================
    ...
    #region Tactical Review Stuff
    // A Review of the Best Actions To Take - Based upon Clear Tactical Observations
    //
    TacticalActions ReviewTacticalActions()
    {
        TacticalActions TheReviewedAction = TacticalActions.None;

        // ======================================================
        // Only Continue of not in any of above states

        if (CurrentlyHasTheBall)
        {
            // Currently has the Ball So the Choices are DribbleForward, DribbleWide, Strike or Pass the Ball
            
            //  Defaults to continue to Dribble forward
            TheReviewedAction = TacticalActions.DribbleForward;

            // However also an Option to Dribble Wide
            if (Random.Range(0, 100) > 50) TheReviewedAction = TacticalActions.DribbleWide;

            if (CheckIfAbleToStrike())
            {
                // Advise a Strike Action if the Direction to the Opposite Goal is Not Blocked 
                if (!CheckIfKickBlocked(TheOppoGoalArea.transform.position)) TheReviewedAction = TacticalActions.StrikeBall;
            }
            else 
            {
                // Consider a Pass if an Oppo Player is Close, the Team Mate is forward, and not blocked            
                if ((CheckIfOppoPlayerIsClose()) && (CheckIfTeamMateIsForward()) && (!CheckIfKickBlocked(TeamPlayer.transform.position)))  TheReviewedAction = TacticalActions.Pass_Ball;
            }
        ...
        {
            // Does Not Have the Ball - So the Choices are ChaseBallPlayer, Goto_OpsPos (Either Strike, or Defend Target Positions)  Depending upon CurrentBallSide
            // Will Also need to Assign the TargetOpPosition

            if (CurrentBallSide== PlayerSide.None)
            {
                // Ball is Free
                if (CheckIfClosestToBall()) TheReviewedAction = TacticalActions.ChaseBallPlayer;
                else
                {
                    // Choose Any Target Position (Strike or Defend positions)  Reagradless of which Side 
                    if(Random.Range(0, 100)>50) TheReviewedAction = TacticalActions.Goto_Attack;
                    else TheReviewedAction = TacticalActions.Goto_Defence;
                }
            } // Ball is free
            // =========================================
            if (CurrentBallSide!= ThePlayerSide)
            {
                // Ball is owned by an Oppo Player
                bool BallWithOppoGoalKeeper = false;
                if ((TheCurrentBallOwner == 13) || (TheCurrentBallOwner == 23)) BallWithOppoGoalKeeper = true; 

                if ((CheckIfClosestToBall()) && (!BallWithOppoGoalKeeper)) TheReviewedAction = TacticalActions.ChaseBallPlayer;
                else
                {
                    // Need to Choose a Defensive Op Position  
                    TheReviewedAction = TacticalActions.Goto_Defence; 
                }
            } // Ball is Free
            if (CurrentBallSide == ThePlayerSide)
            {
                // Always go into a Forward Strike Position as don't have the Ball, but Team Mate does
                TheReviewedAction = TacticalActions.Goto_Attack;
               
            }  // Ball is owned by Team Mate
        }  // Player Does Not Have Ball

        // If Still No Action, then Target Location is current
        if (TheReviewedAction == TacticalActions.None) TargetOpPosition = transform.position; 

        return TheReviewedAction; 
    ...
    public void ConfirmedBallOwner(int CurrentBallOwner)
    {
        ...
        else
        {
            ...
            if (UnderManualControl)
            {
                if ((ThePlayerCurrentState == PlayerState.DribblingForward) || (ThePlayerCurrentState == PlayerState.IdleAtPosition)) SetRunningToBall();
            }
        }
    } // ConfirmedBallOwner
    // ===========================================================================================
    ...
    {
        // Dribble towards the Opposite Gaol
        ThePlayerCurrentState = PlayerState.DribblingForward;
